...
{
    public static class APACNonRes
    {
        ...
        private static string GenerateQuery()
        {
            ...
            string Rule9 = "(Account.ShippingCountry != 'Australia' AND Account.ShippingCountry != 'India')";
            string Rule10 = "(NOT (Account_Name_ReadOnly__c LIKE '%PAPT Spirent Backline%'))";

            return $"SELECT Id FROM Case WHERE ((({Rule1} AND {Rule8} AND {Rule9} AND (({Rule2} AND {Rule3} AND {Rule4}) OR ({Rule5} AND {Rule6} AND {Rule7})))) AND {Rule10})";
        }
