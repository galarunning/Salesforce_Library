...
{
    public static class AmericasCommercial
    {
        ...
        private static string GenerateQuery()
        {
            ...
            string Rule10 = "(Account_Region__c != 'APAC')";
            string Rule11 = "(NOT (Account_Name_ReadOnly__c LIKE '%PAPT Spirent Backline%'))";

            return $"SELECT Id FROM Case WHERE ({Rule1} AND {Rule8} AND (({Rule2} AND {Rule3} AND ({Rule4} OR ({Rule9} AND {Rule7})) AND {Rule10}) OR ({Rule5} AND {Rule6} AND {Rule7}))) AND {Rule11}";
        }
