...
namespace SalesforceEngine.Dashboards
{
    public static class EMEA
    {
        ...
        private static string GenerateQuery()
        {
            ...
            string Rule4 = "(Owner.Name = 'Backline EMEA Queue')";
            ...
            string Rule7 = "(Account_Region__c = 'EMEA')";
            string Rule8 = "(NOT (Account_Name_ReadOnly__c LIKE '%PAPT Spirent Backline%'))";

            return $"SELECT Id FROM Case WHERE ({Rule1} AND (({Rule2} AND {Rule3} AND {Rule4}) OR ({Rule5} AND {Rule6} AND {Rule7}))) AND {Rule8}";
        }
